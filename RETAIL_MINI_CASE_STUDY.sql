...
SELECT MONTH(ORDER_DATE) AS MONTH_DATE, SUM(SALES) AS TOTAL_SALES
FROM RETAIL_ORDERS
GROUP BY MONTH_DATE
ORDER BY MONTH_DATE;
...
-- SELECT PRODUCT_NAME,
    -- REPLACE (PRODUCT_NAME,' ','_') AS PRODUCTS
    -- FROM RETAIL_ORDERS;
    ...
    SELECT ORDER_ID,
         ORDER_DATE,
         DATE_ADD(LAST_DAY(ORDER_DATE),
         INTERVAL 5 DAY) AS REPORTING_DATE
         FROM RETAIL_ORDERS;
...
SELECT ORDER_ID,
   ORDER_DATE,
DATEDIFF(LAST_DAY(ORDER_DATE),ORDER_DATE) AS DAYS_REMAINING
FROM RETAIL_ORDERS;
...
SELECT ORDER_ID,
		SHIP_DATE,
        DATE_SUB(SHIP_DATE, INTERVAL 3 DAY)
        AS REMAINDER_DATE
        FROM RETAIL_ORDERS;
...
SELECT ORDER_ID,
     ORDER_DATE,
     DATE_ADD(LAST_DAY(ORDER_DATE),
	INTERVAL 1 DAY) AS FIRST_DAY_NEXT_MONTH
FROM RETAIL_ORDERS;
...
SELECT ORDER_ID,
         ...
         SHIP_DATE
FROM RETAIL_ORDERS
WHERE SHIP_DATE > DATE_ADD(ORDER_DATE,
INTERVAL 5 DAY);

SELECT CUSTOMER_NAME,
     CONCAT(
      LEFT(SUBSTRING_INDEX(CUSTOMER_NAME,' ', 1),1),
       LEFT(SUBSTRING_INDEX(CUSTOMER_NAME,' ', -1),1) ) AS INITIALS 
        FROM RETAIL_ORDERS;
...
SELECT 
    ...
    SUM(SALES) AS total_sales,
    SUM(profit) AS total_profit,
    ROUND((SUM(profit)) * 100, 2) 
    AS profit_margin,
    CASE 
        WHEN (SUM(profit) ) * 100 > 25 
            THEN 'Excellent'
        WHEN (SUM(profit) ) * 100 
             BETWEEN 15 AND 25 
            THEN 'Good'
